...
object Recap {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // codeblocks evaluate to the value of the last expression
    val aCodeBlock = {
      if (aCondition) 54
      78
    }
    ...
    class Crocodile extends Animal with Carnivore {
      override infix def eat(a: Animal): Unit = println(s"I'm a croc, I'm eating this $a!")
    }
    ...
    val  two = incrementer(1) // incrementer.apply(1)

    // lambdas
    val anonymousIncrementer = (x: Int) => x + 1
    // hofs = higher-order functions
    val anIncrementedList = List(1,2,3).map(anonymousIncrementer) // [2,3,4]
    // map, flatMap, filter
    ...
    // val pairs = List(1,2,3).flatMap(number => List('a', 'b').map(char => s"$number-$char"))

    // Scala collections: Seq, Arrays, Lists, Vectors, Maps, Tuples, Sets
